...
function GenerateLocalizationData() {
  ...
  var UnestableEnergy = specific_1.Ability(
    "Unestable Energy",
    "storm_second_attack.png"
  );
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Storm attacks with lightining balls dealing " +
        specific_1.Keywords.BaseDamage +
        " to the enemy. If hits a target when charged, deals @{ability_damage} damage in area and applies " +
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Storm attacks with an unestable ball of energy that deals @{ability_damage} to the enemy.",
      level:
        "Applies a debuff that deals @{damage_per_tick} damage and applies " +
        specific_1.Keywords.Stun.instance() +
        " for @{stun_duration} every @{think_interval}s. Lasts for @{duration}s.",
    }),
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      ...
      level:
        "Increases the mana gain percentage. Can gain mana from multiple targets. Also drains mana from the enemies.",
    }),
    ability_specials: [
      {
        ability_special: "mana_gain_pct",
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Storm Spirit becomes volatile electricity, charging across the battlefield. Deals @{ability_damage} damage to the enemies in the way, and @{aoe_damage} damage to enemies on the arrival area. Also applies " +
        specific_1.Keywords.FadingSlow.instance("@{fading_slow_pct}") +
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Buff your next " +
        BallLightning +
        ", adding @{damage_per_level} damage and healing yourself for half of the damage dealt with it. If you cast it again while buffed, it will add @{damage_per_level} extra damage and inflict " +
        specific_1.Keywords.Stun.instance() +
        " for @{stun_duration}s.",
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Creates an explosively charged image of Storm Spirit that lasts @{duration} seconds and will detonate and deal @{ability_damage} damage and applies " +
        specific_1.Keywords.Sleep.instance() +
        " for @{sleep_duration}s to enemies that comes near.",
