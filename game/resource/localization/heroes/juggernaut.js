...
function GenerateLocalizationData() {
  ...
  var BladeDance = specific_1.Ability(
    "Blade Dance",
    "juggernaut_second_attack.png"
  );
  var Preparation = specific_1.Ability("Preparation", "juggernaut_counter.png");
  var Swiftslash = specific_1.Ability(
    "Swiftslash",
    "juggernaut_ex_counter.png"
  );
  var YurneroDash = specific_1.Ability(
    "Yurnero Dash",
    "juggernaut_counter_recast.png"
  );
  var BladeMastery = specific_1.Ability(
    "Blade Mastery",
    "juggernaut_basic_attack.png"
  );
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Juggernaut attacks with his weapon dealing " +
        specific_1.Keywords.BaseDamage +
        " to the enemy. Every successful attack generates a " +
        specific_1.Keywords.FuryStack.instance() +
        " and reduces the cooldown of " +
        BladeDance +
        ".",
      level:
        "Successful hits also reduces the cooldown from " +
        Preparation +
        " and " +
        Swiftslash +
        ".",
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Juggernaut swings his blade dealing @{damage_multiplier_per_stack} times your " +
        specific_1.Keywords.BaseDamage +
        " to all enemies in front of you. Consumes all his current " +
        specific_1.Keywords.FuryStack.instance() +
        " to increase the damage multiplier by @{damage_multiplier_per_stack} per stack. If 4 stacks are consumed, a spin attack is cast instead, dealing damage to all enemies around Juggernaut",
      level:
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Goes into counter state for a short period of time. During which, if Juggernaut is hit by a melee attack or projectile, he reflects the attack and gains " +
        YurneroDash +
        " as a recast of this spell.",
      level: "Increases the times that " + YurneroDash + " can be recasted.",
      subAbility: {
        name: "Yurnero Dash",
        iconPath: "juggernaut_counter_recast.png",
        description:
          "Performs a range slash attack towards the cursor location. If the slash attacks hits an enemy, it deals @{ability_damage} damage and allows " +
          YurneroDash +
          " re-casted once again. Can be re-casted up to @{recasts} times.",
      },
    }),
  });
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Throws a dagger that deals @{ability_damage} damage and applies " +
        specific_1.Keywords.FadingSlow.instance("@{fading_slow_pct}") +
        " for @{fading_slow_duration}s on the target.",
      level:
        "When hitting an enemy, can be recasted to instantly teleport behind him.",
    }),
  ...
  Abilities.push({
    ...
    name: "Omnislash",
    description:
      'Juggernaut very quickly leaps towards the mouse location. If an enemy is hit during the dash, Juggernaunt will start lunging between random targets around him, dealing <b><font color="#ffffff">base damage</font></b> every hit. The time between attacks is based on Juggernaut\'s attack rate. Juggernaut is invulnerable for the duration. Ability ends after there is no valid nearby target or after @{duration}s has passed. Consumes all ' +
      specific_1.Keywords.FuryStack.instance() +
      " to increase Juggernaut's attack speed by @{aspd_per_stack} per stack consumed for the ability duration.",
    lore: specific_1.Keywords.FadingSlow.definition,
    ability_specials: [
      {
        ability_special: "AbilityCastPoint",
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      ...
      level:
        "When invoked, the totem gives a shield that blocks @{shield} damage to all nearby allies.",
    }),
    ability_specials: [
      {
        ability_special: "AbilityCastPoint",
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Performs a range slash attack towards the cursor location. If the slash attacks hits an enemy, it deals @{ability_damage}, imbues your " +
        BladeMastery +
        " with extra @{extra_damage} for the next attack and grants you " +
        specific_1.Keywords.Swiftness.instance("@{swiftness_pct}") +
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Performs a meele slash attack that deals @{ability_damage} damage and applies " +
        specific_1.Keywords.Sleep.instance() +
        " for @{duration}s. Consumes all " +
        specific_1.Keywords.FuryStack.instance() +
        " to increase the sleep duration in @{duration_per_stack}s per stack consumed.",
      level:
        "When hitting at least one enemy, replenish all " +
        specific_1.Keywords.FuryStack.instance() +
        ", refunds the mana spend and gives " +
        specific_1.Keywords.Swiftness.instance("@{swiftness_pct}") +
        " for @{swiftness_duration}s</font></b>.",
    }),
    lore: specific_1.multipleKeywords([
      specific_1.Keywords.FuryStack.definition,
      specific_1.Keywords.Sleep.definition,
      specific_1.Keywords.Swiftness.definition,
    ]),
    ability_specials: [
      {
        ability_special: "AbilityCastPoint",
  ...
  Modifiers.push({
    ...
    description:
      "Blocks {" +
      "MODIFIER_PROPERTY_TOOLTIP" /* TOOLTIP */ +
      "} incoming damage.",
  });
  Modifiers.push({
    ...
    name: "Swiftslash",
    description:
      "Your next basic attack deals {" +
      "MODIFIER_PROPERTY_PREATTACK_BONUS_DAMAGE" /* PREATTACK_BONUS_DAMAGE */ +
      "} extra damage.",
  });
  Modifiers.push({
  ...
  Modifiers.push.apply(
    Modifiers,
    specific_1.Upgrades(__spreadArray(__spreadArray([], Shards), Favors))
  );
