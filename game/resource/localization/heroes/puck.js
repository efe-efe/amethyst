...
function GenerateLocalizationData() {
  ...
  var EndlessMisschief = specific_1.Ability(
    "Endless Mischief",
    "puck_second_attack.png"
  );
  var FairyFlare = specific_1.Ability("Fairy Flare", "puck_basic_attack.png");
  var FairyPower = specific_1.Ability(
    "Fairy Power",
    "puck_basic_attack_related.png"
  );
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Range attack that deals " +
        specific_1.Keywords.BaseDamage +
        ". When its charged, Puck gains extra @{charged_damage} base damage and this ability inflicts " +
        specific_1.Keywords.FairyDust.instance("@{fairy_dust_slow_pct}") +
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Fire a orb of light that deals @{ability_damage} on impact. If the target is affected by " +
        specific_1.Keywords.FairyDust.instance() +
        ", also applies " +
        specific_1.Keywords.Silence.instance() +
        " for @{silence_duration}s.",
      level:
        "Increases damage and refreshes " + EndlessMisschief + " cooldown.",
    }),
    lore: specific_1.multipleKeywords([
      specific_1.Keywords.FairyDust.definition,
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Puck Banishes for @{banish_duration}s. When activated, launch a " +
        FairyFlare +
        " towards mouse position. Casting any spell will remove the banish. Has @{max_charges} charges.",
      level:
        "Reduces the cooldown " +
        FairyFlare +
        " before throwing the projectile.",
    }),
    ability_specials: [
      {
        ability_special: "AbilityCastPoint",
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Puck realeses his power in an area. First in a small wave, dealing @{damage_small} damage and applying " +
        specific_1.Keywords.Silence.instance() +
        " for @{silence_duration}s to enemies in area, and then in a bigger wave deaing @{damage} and applying " +
        specific_1.Keywords.FairyDust.instance("@{fairy_dust_slow_pct}") +
        " for @{fairy_dust_duration}s.",
      level:
        "Applies " +
        specific_1.Keywords.Stun.instance() +
        " for @{stun_duration_small}s in the small wave and for @{stun_duration}s in the big wave and increases the damage dealt in both waves. No longer silences",
    }),
    lore: specific_1.multipleKeywords([
      specific_1.Keywords.FairyDust.definition,
      specific_1.Keywords.Silence.definition,
      specific_1.Keywords.Stun.definition,
    ]),
    ability_specials: [
      {
        ability_special: "mana_gain_pct",
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Creates a coil of volatile magic that latches onto enemy Heroes, inflicting a @{initial_stun_duration}s " +
        specific_1.Keywords.Stun.instance() +
        " on them. If the enemy hero stretches the coil by moving too far away, it snaps, inflicting " +
        specific_1.Keywords.Stun.instance() +
        " for @{snap_stun_duration}s, and dealing @{ability_damage} damage. Any time that puck launches a " +
        FairyFlare +
        ", the coil launches one too targeted at the same direction.",
    }),
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Puck teleports himself to the target location and releases a burst of dust that inflicts " +
        specific_1.Keywords.SpellBlock.instance("@{silence_duration}") +
        " for @{pre_silence_duration}s on the enemy units nearby.",
      level:
        "Grants one free recast if " +
        specific_1.Keywords.SpellBlock.instance() +
        " is triggered.",
    }),
    lore: specific_1.multipleKeywords([
      specific_1.Keywords.SpellBlock.definition,
      specific_1.Keywords.Silence.definition,
    ]),
    ability_specials: [
      {
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Puck tranforms himself into his Fairy Form, replacing its " +
        FairyFlare +
        ". Lasts for @{duration}.",
      level:
        "When hitting an enemy with " +
        FairyPower +
        " also heals you for @{heal_per_hit} health.",
      subAbility: {
        name: "Fairy Power",
        description:
          'Ranged area attack that after a delay of @{delay_time}s deals <b><font color="#ffffff">base damage</font></b>. When charged, inflicts ' +
          specific_1.Keywords.FairyDust.instance("@{fairy_dust_slow_pct}") +
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Puck launches a magic orb that floats in a straight path, dealing @{ability_damage} to enemy units along the way. Can be recasted to teleport Puck to the orb's location and deal @{damage_aoe} damage and applies " +
        specific_1.Keywords.Fear.instance() +
        " for @{fear_duration}s in area to enemies on the arrival location.",
