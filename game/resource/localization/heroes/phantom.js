...
function GenerateLocalizationData() {
  ...
  var CriticalStrike = specific_1.Ability(
    "Critical Strike",
    "phantom_second_attack.png"
  );
  var LesserStiflingDagger = specific_1.Ability(
    "Lesser Stifling Dagger",
    "phantom_basic_attack_related.png"
  );
  var PoisonusDagger = specific_1.Ability(
    "Poisonus Dagger",
    "phantom_ex_counter_recast.png"
  );
  var PhantomStrike = specific_1.Ability(
    "Phantom Strike",
    "phantom_counter.png"
  );
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        ...
        ". Successfully hitting a target generate a " +
        specific_1.Keywords.PhantomStack.instance() +
        " and reduces the cooldown of " +
        CriticalStrike +
        " in @{cooldown_reduction}s.",
      level: "",
    }),
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Swings your weapon in a melee attack, dealing " +
        specific_1.Keywords.BaseDamage +
        " to all enemies in front of you. Consumes all " +
        specific_1.Keywords.PhantomStack.instance() +
        " to increase the damage done by @{damage_per_stack} per stack. If you consume 3 stacks, instantly gives " +
        Swift +
        " a charge.",
      level:
        "Fully replenish " +
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        'Counters the next projectile or melee attack. Countering an attack banishes yourself for @{banish_duration} second, after that, teleports you to target location, heals you in <font color="#97DF3B">@{heal}</font>, increases your attack speed by @{as_speed} for @{buff_duration}s and teleports you to a new location.',
      level:
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Throws a dagger that deals " +
        specific_1.Keywords.BaseDamage +
        " to the target plus @{damage_modifier}, inflicts " +
        specific_1.Keywords.FadingSlow.instance("@{fading_slow_pct}") +
        " for @{fading_slow_duration}s and generates a " +
        specific_1.Keywords.PhantomStack.instance() +
        ". You can stack up to @{max_charges} charges.",
      level:
        "Adds another charge, increases the damage and also applies " +
        specific_1.Keywords.Bleeding.instance() +
        ".",
    }),
    lore: specific_1.multipleKeywords([
      specific_1.Keywords.Bleeding.definition,
      specific_1.Keywords.FadingSlow.definition,
      specific_1.Keywords.PhantomStack.definition,
    ]),
    ability_specials: [
      {
        ability_special: "AbilityCastPoint",
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Throws your weapon, dealing @{damage_multiplier} times your " +
        specific_1.Keywords.BaseDamage +
        " and blinking towards the first unit it strikes. The damage mulitplier is increased by @{damage_multiplier_per_stack} per each" +
        specific_1.Keywords.PhantomStack.instance() +
        " consumed.",
      level: "",
    }),
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Dashes towards the pointing direction, dealing @{ability_damage} damage and inflicting " +
        specific_1.Keywords.FadingSlow.instance("@{fading_slow_pct}") +
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Phantom Assassin replaces her basic attack with " +
        LesserStiflingDagger +
        ". Lasts for @{duration}s.",
      subAbility: {
        name: "Lesser Stifling Dagger",
        description:
          "Throws a dagger that deals @{ability_damage} damage to the target, inflicts " +
          specific_1.Keywords.FadingSlow.instance("@{fading_slow_pct}") +
          " for @{fading_slow_duration}s and generates a " +
          specific_1.Keywords.PhantomStack.instance() +
          ".",
      ...
      level:
        "Your daggers applies an stacking " +
        specific_1.Keywords.Bleeding.instance() +
        " debuff on target for @{bleed_duration}s.",
    }),
    lore: specific_1.multipleKeywords([
      specific_1.Keywords.PhantomStack.definition,
      specific_1.Keywords.FadingSlow.definition,
  ...
  Abilities.push({
    ...
    description: specific_1.Description({
      description:
        "Phantom Assassin blurs her body, causing her to be impossible to see and increasing her movement speed in @{speed_buff_pct}% for @{duration}s. During that time, this spell can be recasted to throw a " +
        PoisonusDagger +
        ".",
      subAbility: {
        ...
        description:
          ...
          " for @{sleep_duration}s and generates a " +
          specific_1.Keywords.PhantomStack.instance() +
          ". Consumes all your courrent stacks to increase the duration of " +
          specific_1.Keywords.Sleep.instance() +
          " by @{duration_per_stack}s per stack.",
