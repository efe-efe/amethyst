...
function GameMode.prototype.OnStateEnd(self, state)
    ...
    elseif state == CustomGameState.PRE_LEVEL then
        ...
        if waveGroup then
            self.level = __TS__New(Level, self.alliances, -1, {waveGroup})
        else
            self:EndGame(0)
        end
