...
function modifier_generic_hypnotize:OnCreated( params )
    if IsServer() then
        ...
        self.parent:AddNewModifier(self:GetCaster(), nil, "modifier_generic_silenced_lua", { duration = self:GetDuration() })
...
function modifier_generic_hypnotize:OnDestroy()
    if IsServer() then
        ...
        self.parent:RemoveModifierByName("modifier_generic_silenced_lua")
    end
end
