...
heroes_status["DOTA_ALLIANCE_RADIANT"] = {
    parent_panel: main_panel.FindChildTraverse("HeroesStatusLeft"),
    heroe_panels: {},
}

heroes_status["DOTA_ALLIANCE_DIRE"] = {
...
function AddPlayer( data ){
    ...
    heroes_status[alliance].heroe_panels[data.heroIndex] = $.CreatePanel("Panel", heroes_status[alliance].parent_panel, "");
    heroes_status[alliance].heroe_panels[data.heroIndex].BLoadLayoutSnippet("HeroStatus");
    heroes_status[alliance].heroe_panels[data.heroIndex].FindChildTraverse("HeroStatusAvatarImage").heroname = data.heroName;
    var health_bar_inner = heroes_status[alliance].heroe_panels[data.heroIndex].FindChildrenWithClassTraverse("HeroHealthBarInner")[0]
    ...
    if(localPlayerAlliance == alliance){
        ...
        color = ENEMY_COLOR;
        var health_bar = heroes_status[alliance].heroe_panels[data.heroIndex].FindChildrenWithClassTraverse("HeroHealthBar")[0];
        health_bar.style.height = "100%";

        var mana_bar = heroes_status[alliance].heroe_panels[data.heroIndex].FindChildrenWithClassTraverse("HeroManaBar")[0];
...
function UpdateHealthBar( data ){
    ...
    var health_bar_inner = heroes_status[alliance].heroe_panels[heroIndex].FindChildrenWithClassTraverse("HeroHealthBarInner")[0];
    var health_bar_inner_back = heroes_status[alliance].heroe_panels[heroIndex].FindChildrenWithClassTraverse("HeroHealthBarInnerBack")[0];
...
function UpdateManaBar( data ){
    ...
    var mana_bar_inner = heroes_status[alliance].heroe_panels[heroIndex].FindChildrenWithClassTraverse("HeroManaBarInner")[0];
