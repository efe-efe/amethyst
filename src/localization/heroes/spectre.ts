...
import { abilityWithIcon, upgrade, createKeyword, heal, secondaryAbility } from "../util";
...
const spectralDagger = abilityWithIcon("Spectral Dagger", "spectre_special_attack");
...
localizeAbility("spectre_ex_counter", {
    ...
    description: `After %delay_time%s, drags all enemies around yourself dealing %ability_damage% and healing yourself in ${heal(
        "%heal%"
    )} health and inflicting ${keywords.fadingSlow.name("%fading_slow_pct%%%")} for %fading_slow_duration%s to opponents.`,
...
localizeAbility("spectre_ex_mobility", {
    ...
    upgrade: upgrade(`Also applies ${keywords.fear.name()} for %fear_duration%s to the enemies affected.`),
