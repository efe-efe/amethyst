...
import { abilityWithIcon, createKeyword, heal, image, secondaryAbility, upgrade } from "../util";

const endlessMisschief = abilityWithIcon("Endless Mischief", "puck_second_attack");
...
localizeAbility("puck_second_attack", {
    ...
    upgrade: upgrade(`Increases damage and refreshes ${endlessMisschief} cooldown.`),
...
localizeAbility("puck_counter", {
    ...
    upgrade: upgrade(`Reduces the cooldown ${fairyFlare} before throwing the projectile.`),
...
localizeAbility("puck_special_attack", {
    ...
    lore: [fairyDust.description, keywords.silence.description, keywords.stun.description],
    values: [manaValue, castpointValue]
});
...
localizeAbility("puck_extra", {
    ...
    upgrade: upgrade(`Grants one free recast if ${keywords.spellBlock.name()} is triggered.`),
...
localizeAbility("puck_ex_basic_attack", {
    ...
    upgrade: upgrade(`When hitting an enemy with ${fairyPower} also heals you for ${heal("%heal_per_hit%")} health.`),
