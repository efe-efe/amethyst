...
(function () {
  ...
  function GetActiveAbility(
    heroIndex: EntityIndex
  ): AbilityEntityIndex | undefined {
    ...
    targetIndicatorModifiers.forEach((modifier) => {
      const tempModifier = modifiers.findModifierByName(
        heroIndex as EntityIndex,
        modifier
      );
  ...
  function UpdateTargetIndicator() {
    ...
    if (active) {
      const data: TargetingIndicator | undefined =
        customAbilities.GetTargetingIndicator(active);

      if (data) {
        const heroOrigin = Entities.GetAbsOrigin(heroIndex);
        const mousePositionScreen = GameUI.GetCursorPosition();
        const mousePosition = Game.ScreenXYToWorld(
          mousePositionScreen[0],
          mousePositionScreen[1]
        );
        ...
        if (data.Fixed == 1) {
          ...
          length = math.clamp(
            Game.Length2D(mousePosition, heroOrigin),
            minRange,
            maxRange
          );
        }
        ...
        if (
          data.ModifierName &&
          modifiers.findModifierByName(heroIndex, data.ModifierName)
        ) {
          const modifierIndex = modifiers.findModifierByName(
            heroIndex,
            data.ModifierName
          );
          ...
          if (modifierIndex) {
            if (data.ModifierStacks) {
              const stacks = Buffs.GetStackCount(
                heroIndex,
                modifierIndex as BuffID
              );
        ...
        if (
          ...
          (type == "TARGETING_INDICATOR_AOE" && data.DisplayRange == 1)
        ) {
          if (!particleLine) {
