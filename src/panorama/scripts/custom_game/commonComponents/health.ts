...
export default class Health {
  ...
  constructor(container: Panel, data: HealthData) {
    ...
    this.valuePanel = panels.createPanelSimple(
      container,
      "health__value",
      "Label"
    ) as LabelPanel;
    ...
    this.shieldProgressBar = new ProgressBar(
      "shield__progress-bar",
      this.healthPanel,
      {
    ...
    this.cellsPanel.style.borderRadius = this.rounded
    ...
    if (this.showValue) {
      ...
      this.valuePanel.style.fontFamily =
        "Radiance, FZLanTingHei-R-GBK, TH Sarabun New, Gulim,MingLiU";
    } else {
  ...
  Update(
    ...
    if (shield <= 0) {
      ...
      if (this.shieldOnFront) {
        ...
        const shieldTotalWidth =
          (100 * (maxHealth + shield - health)) / (maxHealth + shield);
        this.shieldProgressBar.SetTotalWidth(shieldTotalWidth);
      }
      ...
      const shieldPanelWidth =
        (100 * (shield + healeableAmount)) / (maxHealth + shield - health);
    ...
    if (this.showValue) {
      ...
      if (this.usePercentage) {
        value =
          Math.floor((100 * potentialHealth) / maxHealth).toString() + "%";
      }
  ...
  UpdateCells(
    health: number,
    healeableAmount: number,
    maxHealth: number,
    shield: number
  ): void {
    const potentialHealthPoints = health + shield + healeableAmount;
    const cellsTotalWidth =
      (100 * potentialHealthPoints) / (maxHealth + shield);
  ...
  AddCell(): void {
    ...
    const panel = panels.createPanelSimple(
      this.cellsPanel,
      "cell_panel_" + index
    );
