...
export default class AllianceBar {
  ...
  constructor(container: Panel, allianceData: AllianceData) {
    ...
    this.healthPanel = panels.createPanelSimple(
      this.panel,
      "alliance-bar__health"
    );
    this.bottomPanel = panels.createPanelSimple(
      this.panel,
      "alliance-bar__bottom"
    );

    const amethystsPanel = panels.createPanelSimple(
      this.bottomPanel,
      "alliance-bar__amethysts"
    );
    const scoreContainerPanel = panels.createPanelSimple(
      this.bottomPanel,
      "alliance-bar__score-container"
    );

    const amethystsImageContainerPanel = panels.createPanelSimple(
      amethystsPanel,
      "alliance-bar__amethysts-image-container"
    );
    const amethystsImagePanel = panels.createPanelSimple(
      amethystsImageContainerPanel,
      "alliance-bar__amethysts-image",
      "Image"
    ) as ImagePanel;
    this.amethystsTextPanel = panels.createPanelSimple(
      amethystsPanel,
      "alliance-bar__amethysts-text",
      "Label"
    ) as LabelPanel;

    this.scorePointsPanel = panels.createPanelSimple(
      scoreContainerPanel,
      "alliance-bar__score-points"
    );

    for (let i = 0; i < 5; i++) {
      const pointPanel = panels.createPanelSimple(
        this.scorePointsPanel,
        "alliance-bar__score-point"
      );
  ...
  public UpdateData(allianceData: AllianceData): void {
    ...
    for (let i = 0; i < allianceData.score; i++) {
      this.scorePointsPanel
        .GetChild(i)!
        .SetHasClass("alliance-bar__score-point--inactive", false);
      this.scorePointsPanel
        .GetChild(i)!
        .SetHasClass("alliance-bar__score-point--active", true);
    }
  }
  ...
  public UpdateMaxScore(maxScore: number): void {
    for (let i = 0; i < maxScore; i++) {
      if (
        !this.scorePointsPanel
          .GetChild(i)!
          .BHasClass("alliance-bar__score-point--active")
      ) {
        this.scorePointsPanel
          .GetChild(i)!
          .SetHasClass("alliance-bar__score-point--disabled", false);
        this.scorePointsPanel
          .GetChild(i)!
          .SetHasClass("alliance-bar__score-point--inactive", true);
      }
    }
  }
