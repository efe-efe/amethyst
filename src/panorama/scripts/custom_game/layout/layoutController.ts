...
export default class LayoutController {
  ...
  private constructor() {
    ...
    this.dimmerPanel = $.CreatePanelWithProperties(
      "Panel",
      this.topPanel,
      "customDimmer",
      {
    ...
    if (entityData) {
      for (let i = 0; i < 9; i++) {
        ...
        if (abilityPanel) {
          ...
          this.UpdateResourceBoxOnAbility(
            abilityPanel,
            abilityManaCost,
            abilityEnergyCost
          );
          ...
          if (abilityEnergyCost > 0) {
            this.UpdateAbilityEnergy(
              abilityPanel,
              abilityEnergyCost,
              (entityData as HeroData).energy
            );
          }
        }
      }
  ...
  public UpdateResourceBoxOnAbility(
    abilityPanel: Panel,
    ...
    if (abilityManaCost > 0 || abilityEnergyCost > 0) {
      ...
      const bevel = abilityPanel.FindChildTraverse(
        "AbilityBevel"
      ) as ImagePanel;
      const image = abilityPanel.FindChildTraverse(
        "AbilityImage"
      ) as ImagePanel;
