...
(function () {
  ...
  const rewardsMainContainerPanel = layout
    .GetTopPanel()
    .FindChildrenWithClassTraverse("rewards")[0];
  const rewardsContainerPanel = layout
    .GetTopPanel()
    .FindChildrenWithClassTraverse("rewards-container")[0];
  const upgradesContainerPanel = layout
    .GetTopPanel()
    .FindChildrenWithClassTraverse("upgrades")[0];
  ...
  const CleanRewards = () => {
    const panels =
      rewardsContainerPanel.FindChildrenWithClassTraverse("reward");
  ...
  const CleanUpgrades = () => {
    const panels =
      upgradesContainerPanel.FindChildrenWithClassTraverse("upgrade");
    panels.forEach((panel) => {
      panel.DeleteAsync(0);
    });
  };
  ...
  const ShowRewards = (rewards: any) => {
    ...
    for (const key in rewards) {
      ...
      const titlePanel = panel.FindChildrenWithClassTraverse(
        "reward__title"
      )[0] as LabelPanel;
      const descriptionPanel = panel.FindChildrenWithClassTraverse(
        "reward__description"
      )[0] as LabelPanel;
      const imagePanel = panel.FindChildrenWithClassTraverse(
        "reward__image"
      )[0] as ImagePanel;
  ...
  const ShowUpgrades = (upgrades: any) => {
    ...
    for (const key in upgrades) {
      ...
      const abilityPanel = panel.FindChildrenWithClassTraverse(
        "upgrade__ability"
      )[0] as AbilityImage;
