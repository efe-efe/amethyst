...
export default class Status {
  ...
  constructor(container: Panel, entityIndex: EntityIndex) {
    ...
    this.textGlowPanel = panels.createPanelSimple(
      topPanel,
      "status__text status__text--glow",
      "Label"
    ) as LabelPanel;
    this.textPanel = panels.createPanelSimple(
      topPanel,
      "status__text",
      "Label"
    ) as LabelPanel;
  ...
  Update(): void {
    if (!this.active || !this.HasData()) {
      ...
      if (modifierIndex) {
        ...
        const style =
          (styles[this.style!] && styles[this.style!]) || styles["Generic"];
