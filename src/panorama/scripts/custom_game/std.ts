...
type PanelDescendants = Exclude<
  keyof PanoramaPanelNameMap,
  "Slider" | "CircularProgressBar" | "SlottedSlider"
>;

type PanoramaOptionsVector = `${number} ${number} ${number}`;

function panelTyped<K extends PanelDescendants>(
  type: K,
  parent: Panel,
  options?: PanelOptions
): PanoramaPanelNameMap[K] {
  const panel = $.CreatePanelWithProperties(
    type,
    parent,
    options?.id ?? "",
    options?.properties ?? {}
...
export namespace std {
  export function image(
    parent: Panel,
    options?: PanelOptions & { src?: string; scaling?: ScalingFunction }
  ) {
  ...
  export function label(
    parent: Panel,
    options?: PanelOptions & { text?: LabelText; html?: boolean }
  ) {
  ...
  export function progressBar(
    parent: Panel,
    options?: PanelOptions & { min?: number; max?: number; value?: number }
  ) {
  ...
  export function scenePanel(
    parent: Panel,
    options?: PanelOptions & { hero?: HeroName }
  ) {
  ...
  export function abilityImage(
    parent: Panel,
    options?: PanelOptions & { ability?: string; scaling?: ScalingFunction }
  ) {
  ...
  export function avatarImage(
    parent: Panel,
    options?: PanelOptions & { steamid?: string }
  ) {
