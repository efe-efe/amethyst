        ...
        GameUI.SetMouseCallback((event, value) => {
            if (event === "pressed"){
                if (value === MouseButtons.LEFT_CLICK && this.onLeftClickPressed){
                    ...
                    const reTry = () => {
                        if (GameUI.IsMouseDown(MouseButtons.LEFT_CLICK) && this.onLeftClickPressed){
                            this.onLeftClickPressed();
                            $.Schedule(0.05, reTry);
                        }
                    };
                ...
                else if (value === MouseButtons.RIGHT_CLICK && this.onRightClickPressed){
                ...
                else if (value === MouseButtons.RIGHT_CLICK && this.onRightClickReleased){
...
const GetActionByActionCode = (actions: Action[], actionCode: number): Action | undefined => {
...
class AbilityAction extends Action {
    ...
    constructor(name: string, actionCode: HeroActions, slotIndex: number, showEffects = true){
...
const CreateCommandByKeyAndActionCodes = (keyCode: KeyCode, actionCode: HeroActions, isAbility = false): Command => {
