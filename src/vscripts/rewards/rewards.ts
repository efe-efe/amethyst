...
class RewardEntity extends BreakableBounty {
    ...
    PlayEffectsOnSpawn(): void {
        ...
        EFX("particles/econ/items/zeus/arcana_chariot/zeus_arcana_blink_start.vpcf", ParticleAttachment.ABSORIGIN_FOLLOW, unit, {
            release: true
        });
        EFX("particles/econ/items/zeus/arcana_chariot/zeus_arcana_blink_end.vpcf", ParticleAttachment.ABSORIGIN_FOLLOW, unit, {
            release: true
        });
...
export const RewardsManager = {
    ...
    GenerateRewards(customNpc: CustomPlayerHeroNPC, options: GenerateRewardsOptions): Reward[] {
        const rewards = Rewards.filter(reward =>
            options.type ? this.ValidateRewardType(reward, options.type) : true && this.ValidateReward(customNpc, reward)
        );
...
CustomGameEventManager.RegisterListener<CustomActionEvent>("custom_npc:select_reward", (eventSourceIndex, event) => {
    ...
    if (player) {
        ...
        if (customNpc) {
            const reward = Rewards.find(reward => reward.type === event.payload.type);
            if (reward) {
