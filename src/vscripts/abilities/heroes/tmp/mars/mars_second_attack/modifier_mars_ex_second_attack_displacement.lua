...
OnCreated(params){
	if(IsServer() ) {
		this.sleep_duration = this.ability.GetSpecialValueFor("sleep_duration")
	}
}

OnDestroy(){
    if(IsServer() ) {
        this.parent.AddNewModifier(this.caster, this.ability, "modifier_generic_sleep", { duration = this.sleep_duration })
    }
}
