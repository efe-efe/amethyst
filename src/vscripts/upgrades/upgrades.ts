...
export const UpgradeManager = {
  ...
  ApplyUpgrade(customNpc: CustomPlayerHeroNPC, upgrade: Upgrade): void {
    if (upgrade.modifier) {
      const ability = upgrade.ability
        ? customNpc.unit.FindAbilityByName(upgrade.ability)
        : undefined;
      customNpc.unit.AddNewModifier(
        customNpc.unit,
        ability,
        upgrade.modifier.name,
        { duration: upgrade.modifier.duration }
      );
  ...
  GenerateUpgrades(
    ...
    const upgrades = Upgrades.filter((upgrade) => {
      ...
      if (
        ...
        this.ValidateUpgradeAlreadyExists(customNpc, upgrade)
      ) {
        return false;
      }
      if (
    ...
    return Math.GetRandomElementsFromArray(
      upgrades,
      Clamp(options.amount, upgrades.length, 0)
    );
  },
  ...
  ValidateUpgradeAbility: (
    ...
    for (let i = 0; i < upgrade.secondaryAbilities.length; i++) {
      ...
      const secondaryAbility =
        customNpc.unit.FindAbilityByName(secondaryAbilityName);
  ...
  ValidateUpgradeAttackCapabilities: (
    ...
    if (
      (upgrade.attackCapability === UnitAttackCapability.MELEE_ATTACK &&
        !customNpc.IsMeele()) ||
      (upgrade.attackCapability === UnitAttackCapability.RANGED_ATTACK &&
        !customNpc.IsRanged())
    ) {
      return false;
    }
  ...
  ValidateUpgradeAlreadyExists: (
    customNpc: CustomPlayerHeroNPC,
    upgrade: Upgrade
  ): boolean => {
    const heroUpgrade = customNpc.heroUpgrades.filter(
      (heroUpgrade) => heroUpgrade.id === upgrade.id
    )[0];
...
CustomGameEventManager.RegisterListener<CustomActionEvent>(
  ...
  (eventSourceIndex, event) => {
    ...
    if (player) {
      ...
      if (customNpc) {
        const upgrade = Upgrades.filter(
          (currentUpgrade) => currentUpgrade.id === event.payload.upgradeId
        )[0];
        if (upgrade) {
