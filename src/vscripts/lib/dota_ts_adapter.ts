...
export interface BaseModifierMotionHorizontal
  extends CDOTA_Modifier_Lua_Horizontal_Motion {}
export class BaseModifierMotionHorizontal extends BaseModifier {}

export interface BaseModifierMotionVertical
  extends CDOTA_Modifier_Lua_Vertical_Motion {}
export class BaseModifierMotionVertical extends BaseModifier {}

export interface BaseModifierMotionBoth
  extends CDOTA_Modifier_Lua_Motion_Both {}
...
setmetatable(BaseModifier.prototype, {
  __index: CDOTA_Modifier_Lua ?? C_DOTA_Modifier_Lua,
});
...
export const registerModifier =
  (name?: string) => (modifier: new () => CDOTA_Modifier_Lua) => {
    if (name !== undefined) {
      // @ts-ignore
      modifier.name = name;
    } else {
...
export function clearTable(table: object) {
...
export function entityFunction(name: string, f: (...args: any[]) => any) {
  const [env] = getFileScope();
