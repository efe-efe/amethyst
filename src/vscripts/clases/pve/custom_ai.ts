...
function CustomAI(
  ...
  function UpdateTarget(): void {
    const currentFollowRange =
      tauntedRemainingDuration > 0 ? tauntedFollowRange : followRange;

    if (followTarget) {
      if (
        CustomEntitiesLegacy.GetDistance(unit, followTarget) <=
          currentFollowRange &&
        !followTarget.IsInvisible()
      ) {
        return;
      } else {
    ...
    const npcAbility: CustomAIAbility = {
      ...
      requirements: options.requirements
        ? {
            ...
            cooldownReady:
              options.requirements.cooldownReady !== undefined
                ? options.requirements.cooldownReady
                : true,
            phaseReady:
              options.requirements.phaseReady !== undefined
                ? options.requirements.phaseReady
                : true,
          }
        : {
            cooldownReady: true,
      ...
      if (npcAbility.requirements) {
        ...
        if (npcAbility.requirements.targetInCastRange) {
          abilityTarget = FindEnemy(
            ability.GetCastRange(Vector(0, 0, 0), undefined)
          );
          if (!abilityTarget) {
            return false;
          }
