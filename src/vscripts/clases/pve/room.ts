...
export default class Room extends GameState {
  ...
  OnRewardSelected(): void {
    ...
    if (
      rewardsReady &&
      this.phases[this.phaseIndex] === RoomPhases.REWARD_OFFERING
    ) {
      this.IncrementPhase();
    }
  }
  ...
  OnRewardApplied(): void {
    ...
    if (
      upgradesReady &&
      this.phases[this.phaseIndex] === RoomPhases.REWARD_CLAIM
    ) {
      this.IncrementPhase();
    }
  }
  ...
  OfferRewards(): void {
    this.GetAllPlayers().forEach((player) => {
      const customNpc = player.customNpc;
      const heroData = this.heroesData.filter(
        (heroData) => heroData.customNpc === customNpc
      )[0];
      if (customNpc && heroData) {
        RewardsManager.OfferRewardsForHero(
          customNpc,
          heroData.nextRewardOfferings
        );
      }
    });
  }
  ...
  ClaimRewards(): void {
    this.GetAllPlayers().forEach((player) => {
      ...
      if (customNpc && customNpc.reward) {
        const rewardEntity = RewardsManager.ClaimRewardForHero(
          customNpc,
          customNpc.reward
        );
        if (rewardEntity) {
  ...
  RemoveSpawner(spawner: Spawner): void {
    spawner.Destroy();
    this.spawners = this.spawners.filter((_spawner) => {
      spawner !== _spawner;
    });
  }
