...
export default class Round extends GameState {
  ...
  constructor(alliances: Alliance[], duration: number) {
    ...
    this.gemSpawnPoints = Entities.FindAllByName("orb_spawn").map((entity) =>
      entity.GetAbsOrigin()
    );
    ...
    this.radiantWarmupSpawn = Entities.FindByName(
      undefined,
      "radiant_warmup_spawn"
    )!;
  ...
  SpawnArrows(): void {
    if (this.gem.entity) {
      ...
      this.gemSpawnPoints.forEach((point, i) => {
        if (i !== this.gem.index) {
          const direction = this.gemSpawnPoints[this.gem.index]
            .__sub(point)
            .Normalized();
          const efx = ParticleManager.CreateParticle(
            "particles/gem_finder.vpcf",
  ...
  CreateGem(): void {
    this.gem.type = RandomIntWithExeption(
      GemTypes.AMETHYST,
      GemTypes.EMERALD,
      this.gem.type
    );
    if (!this.timeOver) {
      this.gem.index = RandomIntWithExeption(
        0,
        this.gemSpawnPoints.length - 1,
        this.gem.index
      );
    }
    const time =
      this.gem.number === 0 ? settings.GemSpawnTime : settings.GemRespawnTime;
  ...
  EndRound(): void {
    ...
    if (this.winner) {
      ...
      if (
        ...
        this.GetHighestWinsDifference(this.winner) >=
          settings.RoundsDifferenceToWin
      ) {
    ...
    this.GetAllPlayers().forEach((player) => {
      ...
      if (hero) {
        CustomEntitiesLegacy.SafeDestroyModifier(
          hero,
  ...
  GetCompetingAlliances(): Alliance[] {
    ...
    this.GetAllPlayers().forEach((player) => {
      if (
        ...
        const alreadyAdded =
          alliances.filter((alliance) => alliance.name === playerAlliance.name)
            .length > 0;
