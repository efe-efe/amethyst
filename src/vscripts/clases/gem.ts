  ...
  Update(): void {
    ...
    if (!CustomEntitiesLegacy.IsBanished(this.GetUnit())) {
      CustomEntitiesLegacy.FullyFaceTowards(
        this.GetUnit(),
        Vector(math.cos(currentAngle), math.sin(currentAngle))
      );
...
class Emerald extends Gem {
  ...
  Effect(killer: CDOTA_BaseNPC) {
    ...
    const final_heal_per_second = this.ProcessValue(
      allies,
      heroes,
      this.heal_per_second
    );
...
export default class GemWrapper {
  ...
  Update(): void {
    ...
    if (
      math.fmod(this.current_time, this.light_delay_time) === 0 &&
      this.lights.length < 5
    ) {
      this.CreateLight();
    }
